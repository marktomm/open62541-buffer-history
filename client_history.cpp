...
static void printUpdateType(UA_HistoryUpdateType type)
{
    switch (type) {
...
static void printTimestamp(char* name, UA_DateTime date)
{
    ...
    if (name)
        printf("%s: %02u-%02u-%04u %02u:%02u:%02u.%03u, ", name, dts.day,
               dts.month, dts.year, dts.hour, dts.min, dts.sec, dts.milliSec);
    else
        printf("%02u-%02u-%04u %02u:%02u:%02u.%03u, ", dts.day, dts.month,
...
#ifdef UA_ENABLE_EXPERIMENTAL_HISTORIZING
static UA_Boolean readRawModified(const UA_HistoryModifiedData* data)
{
...
static UA_Boolean readEvents(const UA_HistoryEvent* data)
{
    ...
    for (size_t i = 0; i < data->eventsSize; ++i) {
        ...
        for (size_t j = 0; j < data->events[i].eventFieldsSize; ++j) {
            printf("Processing %lu: %s\n", (long unsigned)j,
                   data->events[i].eventFields[j].type->typeName);
        }
    }
    return true;
}
...
static UA_Boolean readHist(UA_Client* client, const UA_NodeId* nodeId,
...
int main(int argc, char* argv[])
{
    ...
    retval = UA_Client_HistoryRead_raw(
        client, &node, readHist, UA_DateTime_fromUnixTime(0), UA_DateTime_now(),
        UA_STRING_NULL, true, 100, UA_TIMESTAMPSTORETURN_BOTH, (void*)UA_FALSE);
    ...
    retval = UA_Client_HistoryRead_modified(
        client, &node, readHist, UA_DateTime_fromUnixTime(0), UA_DateTime_now(),
        UA_STRING_NULL, false, 100, UA_TIMESTAMPSTORETURN_BOTH,
        (void*)UA_FALSE);
    ...
    retval = UA_Client_HistoryRead_events(
        client, &eventNode, readHist, UA_DateTime_fromUnixTime(0),
        UA_DateTime_now(), UA_STRING_NULL, filter, 10,
