 ...
 *
 * Copyright 2019 (c) basysKom GmbH <opensource@basyskom.com> (Author: Peter
 * Rustler)
 */
...
static std::vector<int> eventBuffer(1000);
               ...
               UA_DataValue* dataValue)
{
    ...
    if (!isFirst && isSecond) {
        isSecond = false;
        dataValue->sourceTimestamp = UA_DATETIME_UNIX_EPOCH;
        dataValue->hasSourceTimestamp = true;
        dataValue->serverTimestamp = UA_DATETIME_UNIX_EPOCH;
...
{
    readFromBuffer(server, sessionId, sessionContext, nodeId, nodeContext,
                   false, range, const_cast<UA_DataValue*>(dataValue));
}

static void updateVaraible(UA_Server* server, void* /*ctx*/)
{
    UA_Int32 varVal = 0;
    if (eventBuffer.size()) {
        varVal = eventBuffer.back();
        eventBuffer.pop_back();
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "update value to %d",
                    varVal);
    } else {
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "buffer empty");
        UA_Server_removeRepeatedCallback(server, periodicalCbId);
    }
    UA_Variant value;
    UA_Variant_setScalar(&value, &varVal, &UA_TYPES[UA_TYPES_UINT32]);
...
int main(void)
{
     ...
     * The UA_HistoryDataGathering is responsible to collect data and store it
     * to the database. We will use this gathering for one node, only.
     * initialNodeIdStoreSize = 1
     * The store will grow if you register more than one node, but this is
     * expensive. */
    ...
    attr.description = UA_LOCALIZEDTEXT((char*)"en-US", (char*)"myUintValue");
    attr.displayName = UA_LOCALIZEDTEXT((char*)"en-US", (char*)"myUintValue");
    ...
    /* We also set this node to historizing, so the server internals also know
     * from it. */
    attr.historizing = true;

    /* Add the variable node to the information model */
    UA_NodeId uint32NodeId = UA_NODEID_NUMERIC(1, 1);
    UA_QualifiedName uint32Name = UA_QUALIFIEDNAME(1, (char*)"myUintValue");
    UA_NodeId parentNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER);
    UA_NodeId parentReferenceNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES);
    UA_NodeId outNodeId;
    UA_NodeId_init(&outNodeId);
    UA_StatusCode retval = UA_Server_addVariableNode(
        server, uint32NodeId, parentNodeId, parentReferenceNodeId, uint32Name,
    ...
    cb.onWrite = NULL;
    UA_Server_setVariableNode_valueCallback(server, uint32NodeId, cb);

    // UA_DataSource ds;
    // ds.read = readFromBuffer;
    // ds.write = NULL;
    // UA_StatusCode retval = UA_Server_addDataSourceVariableNode(
    //     server, uint32NodeId, parentNodeId, parentReferenceNodeId,
    //     uint32Name, UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
    //     attr, ds, NULL, &outNodeId);
     ...
     * be copied. */
    setting.historizingBackend = UA_HistoryDataBackend_simpleBuffer();
    ...
    /* If you want to insert the values to the database yourself, we can set the
     * user strategy here. This is useful if you for example want a value
     * stored, if a defined delta is reached. Then you should use a local
     * monitored item with a fuzziness and store the value in the callback.
     */
    ...
    retval = gathering.registerNodeId(server, gathering.context, &outNodeId,
                                      setting);
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "registerNodeId %s",
                UA_StatusCode_name(retval));

    // UA_Server_addRepeatedCallback(server, updateVaraible, NULL, 100,
    // &periodicalCbId);
    ...
    // UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "startPoll %s",
    // UA_StatusCode_name(retval));

    retval = UA_Server_run(server, &running);
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "UA_Server_run %s",
                UA_StatusCode_name(retval));
    /*
    ...
    // UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "stopPoll %s",
    // UA_StatusCode_name(retval));
